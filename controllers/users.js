...
module.exports.createNewUser = async (req, res) => {
  try {
    ...
    if(err.name === 'ValidationError') {
      return res.status(INCORRECT_DATA_ERROR).send({message: "При создании пользователя переданы неверные данные!"})
    }
    console.log(`Статус ${err.status}. Ошибка ${err.name} c текстом ${err.message}`)
    return res.status(SERVER_ERROR).send({message: "Внутрення ошибка червера!"})
  }
...
module.exports.updateUserProfile = async (req, res) => {
  try {
    ...
    if(userForUpdate) {
      const updatedUser = await User.findByIdAndUpdate({_id: owner}, {...req.body}, {new: true})
    ...
    if(err.name === 'ValidationError') {
      return res.status(INCORRECT_DATA_ERROR).send({message: "При обновлении пользователя переданы неверные данные!"})
    }
    console.log(`Статус ${err.status}. Ошибка ${err.name} c текстом ${err.message}`)
    return res.status(SERVER_ERROR).send({message: "Внутрення ошибка червера!"})
  }
...
module.exports.updateUserAvatar = async (req, res) => {
  try {
    ...
    const userForUpdate = await User.findById({_id: owner})
    if(userForUpdate) {
      const updatedUser = await User.findByIdAndUpdate({_id: owner}, {avatar: avatar}, {new: true})
      res.send({updatedUser})
    } else {
      throw new Error(`У вас нет прав на редактирование данного пользователя!`)
    }
